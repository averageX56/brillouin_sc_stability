...
__device__ void rhs_device(const double2* a, const double2* b, double E, int n_ph,
    ...
    for (int j = 0; j < n_ph; ++j) {
        ...
        if (j + 1 < n_ph) v = add(v, scale(iz(mul(b[j & 1], a[j + 1])), g));
        if (j > 0) v = add(v, scale(iz(mul(conjz(b[(j - 1) & 1]), a[j - 1])), g));
...
__global__ void integrate_kernel(const double* E, const double2* det_a, const double2* det_b,
    ...
    if (idx >= total) return;
    int q = idx / n_paths;
    double2 a[MAX_MODES], b[2];
    for (int j = 0; j < n_ph; ++j) a[j] = det_a[q * n_ph + j];
    b[0] = det_b[q * 2]; b[1] = det_b[q * 2 + 1];
...
void rhs_host(const std::vector<C>& a, const C* b, double E, const Config& c,
    ...
    for (int j = 0; j < c.n_ph; ++j) {
        C v = -0.5 * c.gamma * a[j];
        if (j + 1 < c.n_ph) v += I * c.g * b[j & 1] * a[j + 1];
        if (j > 0) v += I * c.g * std::conj(b[(j - 1) & 1]) * a[j - 1];
        if (j == 0) v -= I * E;
        da[j] = v;
    }
    for (int p = 0; p < 2; ++p) {
...
void write_json(const Config& c, const std::vector<PumpResult>& results, int n_keep,
    ...
    o << "{\n\"meta\":{\"backend\":\"cuda_cpp\",\"device\":\"" << prop.name
    ...
    o << "},\n\"params\":{\"ORDER\":"<<c.n_ph<<",\"N_PHON\":2,\"G\":";write_number(o,c.g);
    ...
    std::vector<double> null_a(c.n_ph,NAN), null_b(2,NAN);
...
int auto_chunk(const Config& c, int n_keep) {
    ...
    size_t per = static_cast<size_t>(c.n_paths)*c.n_ph*n_keep*sizeof(double2)
               ...
               + static_cast<size_t>(c.n_ph + 2)*sizeof(double2) + sizeof(double);
...
int run(const Config& c) {
    ...
    std::vector<C> a(c.n_ph,0.0); C b[2]={0.0,0.0};
    std::vector<double2> detA(c.nE*c.n_ph),detB(c.nE*2);
    for(int q=0;q<c.nE;++q){R[q].E=c.E_list[q];R[q].steady=deterministic_state(R[q].E,c,a,b);
      R[q].A_det.resize(c.n_ph);R[q].B_det.resize(2);
      for(int j=0;j<c.n_ph;++j){R[q].A_det[j]=std::abs(a[j]);detA[q*c.n_ph+j]=z(a[j].real(),a[j].imag());}
      for(int p=0;p<2;++p){R[q].B_det[p]=std::abs(b[p]);detB[q*2+p]=z(b[p].real(),b[p].imag());}}
    ...
    for(int start=0;start<c.nE;start+=chunk_size){int nq=std::min(chunk_size,c.nE-start);
      ...
      cuda_check(cudaMalloc(&dB,nq*2*sizeof(double2)),"malloc detB");cuda_check(cudaMalloc(&dS,ns*sizeof(double2)),"malloc samples");
      ...
      cuda_check(cudaMemcpy(dB,detB.data()+start*2,nq*2*sizeof(double2),cudaMemcpyHostToDevice),"copy detB");
      ...
      for(int q=0;q<nq;++q){auto&r=R[start+q];r.A_mean.assign(c.n_ph,NAN);r.B_mean.assign(2,NAN);r.g2_a.assign(c.n_ph,NAN);r.g2_b.assign(2,NAN);r.fwhm_g1.assign(c.n_ph,NAN);r.r2_g1.assign(c.n_ph,NAN);
        unsigned long long cnt=0;std::vector<double>sa(c.n_ph),s2(c.n_ph),s4(c.n_ph);double sb[2]={},b2[2]={},b4[2]={};
        for(int pth=0;pth<c.n_paths;++pth){const auto&m=hm[q*c.n_paths+pth];if(m.diverged){++r.n_diverged;continue;}cnt+=m.count;
          ...
          for(int p=0;p<2;++p){sb[p]+=m.abs_b[p];b2[p]+=m.i2_b[p];b4[p]+=m.i4_b[p];}}
        if(cnt){for(int j=0;j<c.n_ph;++j){r.A_mean[j]=sa[j]/cnt;double m2=s2[j]/cnt;r.g2_a[j]=m2>0?(s4[j]/cnt)/(m2*m2):NAN;
          ...
          for(int p=0;p<2;++p){r.B_mean[p]=sb[p]/cnt;double m2=b2[p]/cnt;r.g2_b[p]=m2>0?(b4[p]/cnt)/(m2*m2):NAN;}}
